pub const RESP_READY: &[u8] = b"HTTP/1.1 204 No Content\r\nContent-Length: 0\r\nConnection: keep-alive\r\n\r\n";
...
mod tests {
    ...
    fn assert_content_length(resp: &[u8]) {
        ...
        let cl_line = header.lines().find(|l| l.starts_with("Content-Length:")).unwrap();
        let cl: usize = cl_line.split(':').nth(1).unwrap().trim().parse().unwrap();
        assert_eq!(cl, body.len(), "header CL {} != body {}", cl, body.len());
    }

    #[test] fn cl_s0() { assert_content_length(RESP_APPROVED_S0); }
    #[test] fn cl_s2() { assert_content_length(RESP_APPROVED_S2); }
    #[test] fn cl_s4() { assert_content_length(RESP_APPROVED_S4); }
    #[test] fn cl_s6() { assert_content_length(RESP_DENIED_S6); }
    #[test] fn cl_s8() { assert_content_length(RESP_DENIED_S8); }
    #[test] fn cl_s10() { assert_content_length(RESP_DENIED_S10); }
}
